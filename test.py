...
def main(args):
    ...
    for img in X_test:
        img = (img.reshape(
            1, img.shape[0], img.shape[1], 1)).astype(float)
        predicted = srcnn.process(img)
        predicted_list.append(predicted.reshape(
            predicted.shape[1], predicted.shape[2], 1))
    n_img = len(predicted_list)
    dirname = './result2'
    for i in range(n_img):
        imgname = 'image{:02}'.format(i)
        cv2.imwrite(os.path.join(dirname, imgname +
                                 '_original.bmp'), X_pre_test[i])
        ...
        cv2.imwrite(os.path.join(dirname, imgname +
                                 '_predicted.bmp'), predicted_list[i])
